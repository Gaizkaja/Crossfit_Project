...
$(document).ready(function () {
    atletas();//Carga los atletas
    selectCategoria();//Carga el select del insert y el update
    //Cuando haces click en Insert, primero validamos el formulario y despues ejecutamos el insert
    $("#btnExecInsert").click(function () {
        if (validarInsert()) {
            execInsert();
        }
    });
    //Cuando haces click en Update, primero validamos el formulario y despues ejecutamos el Update
    ...
    //Confome cambia los datos de la edad, cambiamos automaticamente la categoria y activamos el select de entrenadores
    ...
    //Cuando subimos una foto, rellenamos el savedFileBase64 y el filename
...
function changeBodyClaroAtletas() {
    ...
    if (modo === "Modo Oscuro") {
        ...
        $('nav').css('background-color', '#0e0e0e'); // YEEEEEEEEET
        $("h1").css('color', 'white');
...
function atletas() {
    ...
    fetch(url, {
        ...
        for (let i = 0; i < atletas.length; i++) {
            ...
            const photoPath = '../uploads/' + atletas[i].foto;

            newRow += "<div class='card col-12 col-md-4 col-lg-3 '>"
                + "<div class='rounded-circle profile' style='background-image: url(\"" + photoPath + "\")'>"
                + "</div>"
                + "<div class='card-body'>"
        ...
        fetch('../controller/cCheckSession.php')
            ...
            .then(response => {
                if (!eval(response.answer.admin)) {
                    $('.card-body button, .container-fluid > div > button').addClass('d-none').attr('disabled', 'true');
                }
        ...
        .then(result => {
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header d-flex justify-content-between">${entrenador[0].nombre}&nbsp ${entrenador[0].apellido}&nbsp->&nbspLvl:${entrenador[0].nivel}&nbsp->&nbspExp:${entrenador[0].experiencia}</span>
...
function activarSelect(op) {
    ...
    if (op) {
        ...
        if ($("#ageUpdate").val() > 0 && $("#ageUpdate").val() <= 16) {
            $('#categoryUpdate').val(1);
            $('#trainerUpdate').prop('disabled', false);
            selectEntrenador(1);
        }
        if ($("#ageUpdate").val() > 16 && $("#ageUpdate").val() < 50) {
